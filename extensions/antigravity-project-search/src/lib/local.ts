...
export async function getLocalProjects(projectRoots: string): Promise<string[]> {
  ...
  // Designed for large roots like ~/Documents:
  // limit both depth and number of collected projects to stay within memory limits.
  const MAX_DEPTH = 6;
  ...
  async function scanDirectory(currentPath: string, depth: number): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      // Treat every subdirectory as a potential project, regardless of .git.
      projects.add(fullPath);

      // Continue scanning deeper levels
